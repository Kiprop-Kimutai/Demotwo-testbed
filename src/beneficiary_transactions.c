...
{
	int selected = 0;
	char  beneficiary_menu[][100] = {"Pay Merchant", "Cash  Out","Beneficiary  Account Operation", "Card Issuance"};

	while(1)
	{


		switch( lcd_menu("Beneficiary Transactions", beneficiary_menu, sizeof(beneficiary_menu)/100 ,selected))
		{
...
{
	int selected = 0;
	char  beneficiary_card_operation[][100] = {"Balance Inquiry","Update Programmes","Mini Statement"};

	while(1)
	{
		switch(lcd_menu("Beneficiary Card Operation", beneficiary_card_operation, sizeof(beneficiary_card_operation)/100 ,selected))
		{
		case 0:
...
char  ** transaction_limit(char  * transaction_date , double transation_amout)
{
	char   transactions[2][50];
...
/*			cJSON * card_transaction_date ;
			char * todays_date;
			cJSON * card_transaction_amount;
			int todays_transactions;

			card_transaction_date = cJSON_GetObjectItem(transaction_json , "date");
			if(card_transaction_date != (cJSON  *) NULL)
			{
				card_transaction_amount = cJSON_GetObjectItem(transaction_json , "amount");
			...
			if(cJSON_GetArraySize(balance_information) == 0){
				message_display_function(1, "","Beneficiary Transaction error" ,  "The card has no available balances. Please get update card online and try  again ",  (char *) NULL);
				kb_getkey();
				return;
			}

			if(atoi(cJSON_Print(cJSON_GetObjectItem(transaction_json , "txnC")))>9){
				message_display_function(1, "","Beneficiary Transaction error" ,  "Please update card to  post all transactions before proceeding with this transaction" ,  (char *) NULL);
				kb_getkey();
				return;
			}

/*			if(atoi(cJSON_Print(cJSON_GetObjectItem(transaction_json , "transacted")))>9){
				message_display_function(1, "","Beneficiary Transaction error" ,  "Please update card to  post all transactions before proceeding with this transaction" ,  (char *) NULL);
				kb_getkey();
				return;
			}*/
					...
					if( strlen(getCharacters)>0 && ret!=-1)
					{
						if(atof(getCharacters) < 70000)
						{
							...
							for (u  = 0  ;  u  < 2 ;  u ++)
							{
								...
								if(u == 0)
								{
									cJSON_AddStringToObject(array_item ,  "key" , "Wallet" );
									cJSON_AddStringToObject(array_item ,  "value" , wallet_id[selected] );
								}
								if ( u ==1)
								{
									cJSON_AddStringToObject(array_item ,  "key" , "Amount" );
									cJSON_AddStringToObject(array_item ,  "value" , getCharacters );
								}
								cJSON_AddItemToArray( confirm_tx_json , array_item);
							}
							printf("Confirm  S :  %s \n" ,  cJSON_Print(confirm_tx_json));
							if (confirm_screen("Confirm Transaction" , confirm_tx_json)) {
								cJSON * txToPosted;
								cJSON *  json_array;
								if(wallet_amount[selected]>=atof(getCharacters)){
									...
									if(resp){
										...
										if(resp ==2)
										{
											...
											cJSON_AddNumberToObject(new_transaction_json ,"txnL" ,0);
											cJSON_AddNumberToObject(new_transaction_json ,"txnC" ,tx_count);
										...
										else
										{
											...
											cJSON_AddNumberToObject(new_transaction_json ,"txnL" ,0);
											cJSON_AddNumberToObject(new_transaction_json ,"txnC" ,0);
										...
										char  * strl = cJSON_Print(new_transaction_json);
										cJSON_Minify(strl);
								...
								//clear all params
								message_display_function(1, "","Beneficiary Transaction error" ,  "The requested transaction amount is more than the card balance available", (char *)NULL);
								kb_getkey();
						...
						{
							message_display_function(1, "","Beneficiary Transaction error" ,  "The requested transaction amount is more than allowed card transaction limit. Please try  again" , (char *)NULL);
							kb_getkey();
						}
					}
		...
		if(send_gprs_request("RegisterBen", card_update_request_json ,"/api/beneficiaryrequest/programmeupdate" ,&response ,  1)){
			...
			if(strcmp(cJSON_Print(cJSON_GetObjectItem(response ,"status")),  "true") == 0)
			{
				...
				if(balances_found_on_server)
				{
					...
					cJSON_AddItemToObject(final_tx ,"txnL" , 0);
					cJSON_AddItemToObject(final_tx ,"txnC" , 0);
					...
					char  * strl = cJSON_Print(final_tx);
					cJSON_Minify(strl);

					cJSON_ReplaceItemInObject(final_tx ,"txnL" ,strlen(strl));
	...
	if(returned > 0){
		...
		if(fplib_test(1))
		{
			if(strlen(existing_transaction) == 0)
			{
				...
				cJSON * returned_balances;
				//if Post and get_feedback is ok
				resp = post_transaction_file("card",existing_transaction, &returned_balances ,  1 ,  &tx_count);
				if(resp){
					cJSON * new_transaction_json = cJSON_CreateObject();
					char  * data_to_be_written;
					char * final_data_to_be_written;
			...
			if(strlen(existing_transaction) == 0)
			{
				...
				if(resp){
					...
					else
					{
						...
						cJSON_AddItemToObject(new_transaction_json ,"txnL" ,0 );
						cJSON_AddItemToObject(new_transaction_json ,"txnC" ,0 );
...
void print_beneficairy_card_balances( void){
	...
	if(verify_card_and_read_data( &transaction_json , &json_beneficiary_data)){
		...
		char  * date, * unformatedDate, * tx_id;
		get_date_and_receipt( 0 , &date ,&unformatedDate,  &tx_id);
		printf("transaction_json  :  %s" ,cJSON_Print(transaction_json) );
		card_balance_information_json  = cJSON_GetObjectItem(transaction_json , "balances");
		if(card_balance_information_json != (cJSON *)NULL)
		{
			existing_transaction = get_string_from_jason_object(cJSON_Print(cJSON_GetObjectItem(transaction_json , "transactions")));
...
int  post_transaction_file(char  * transaction_source, char * transaction , cJSON ** response_balances  , int display_messages ,  int *  transaction_count)
{
	//char  ** tokens  = 0 ;
...
int verify_card_and_read_data(  cJSON ** transaction_file , cJSON ** beneficiary_details){
	...
	if(returned == 1){
		...
		if(p_details != (char *) NULL  && tx_file != (char *) NULL)
		{
			char * temp =  malloc(strlen(p_details)+2);
			char * temp2 =  malloc(strlen(tx_file)+2);
			strcpy(temp ,p_details );
			strcpy(temp2 ,tx_file );
			if(fplib_test(1 ))
			{
				//*cardno = "12335663232";
				//printf("p_details : %s \n" , p_details);
				...
				if(jcheck(temp))
				{
					*beneficiary_details=  cJSON_Parse(temp);
					if(jcheck(temp2))
					{
						*transaction_file   = cJSON_Parse(temp2);
...
int post_pos_offline_transactions (void)
{
	...
	if(sql_data_count)
	{
		...
		strcat(new_transaction ,"#");
		printf("Final :  %s\n" , new_transaction);
		if(post_transaction_file("pos" ,new_transaction  , &response  , 0 ,  &tx_count))
		{
