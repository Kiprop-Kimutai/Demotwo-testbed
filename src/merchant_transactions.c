...
{
	int selected = 0;
	const char  merchant_menu[][100] = {"Store Balance Inquiry", "Store Mini Statement", "Store Roll-Up Transfer"};
	while(1)
	{
				...
				if(strcmp(status , "true")==0){
					...
					if(cJSON_GetArraySize(walletsresp) > 0){
						...
						while( i<cJSON_GetArraySize(walletsresp)){
							...
							defined_x = 100;
							sprintf(buffer, "MINI-STATEMENT: %d/%d", i+1, cJSON_GetArraySize(walletsresp));
							...
							lcd_set_font_color(COLOR_BLACK);
							 key = kb_getkey();
							if(key == DIKS_ENTER){
								print_receipt("STORE MINI-STATEMENT", walletsresp,&print_flag, &print_complete);
								return 0;
							}
				...
				if(strcmp(status, "true")==0){
					...
					if(cJSON_GetArraySize(walletsresp) > 0){
						...
						if(confirm_screen("STORE BALANCE" , confirm_tx_json))
						{
							...
							print_receipt("STORE BALANCE", walletsresp,&print_flag, &print_response);
